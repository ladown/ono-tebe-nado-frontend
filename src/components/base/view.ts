...
export class Partial<NodeType extends HTMLElement, DataType extends object, Events extends string, Modifiers> {
	...
	protected element<T extends Partial<any, any, any, any>>(
		name: string,
	...
	protected select<T extends Partial<any, any, any, any>>(
		name: string,
		selector?: string,
		ClassType?: new (el: HTMLElement, name: string) => T,
	): T {
		if (!this.elements[name] && selector) {
	...
	static mount<T>(selectorElement: HTMLElement | string, data?: any, name?: string): T {
		const element = typeof selectorElement === 'string' ? document.querySelector(selectorElement) : selectorElement;
		return this.factory(element, data, name) as T;
	}
}
